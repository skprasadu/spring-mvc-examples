...
public abstract class UiRuleValidatorService {
	...
	protected abstract FormInformation findTemplateByName(String appName, String formId);
	protected abstract UiRule findRule(String name);
	
	public List<String> evaluate(String appName, String formId, LinkedHashMap<String, String> mapa) throws InstantiationException, IllegalAccessException,
			ClassNotFoundException, ParseException {
				// This is the Drools rules part.
				// We also need to get the input
				FormInformation info = findTemplateByName(appName, formId);
				// Get Input information for the rule
				String inputs = info.getRuleDetails().get(0).getValue();
				// Get Drools for the same rules
				UiRule uiRule = findRule(info.getRuleDetails().get(0).getName());
